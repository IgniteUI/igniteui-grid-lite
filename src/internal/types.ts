...
type DotPaths<T, Depth extends number = 5> = {
  [K in keyof T & string]: Depth extends 0
    ? K
    : T[K] extends object
      ? T[K] extends NonTraversable
        ? K
        : K | `${K}.${DotPaths<T[K], Prev[Depth]>}` // Note: resolving `never` will collapse the entire interpolated string to never, leaving only valid paths
      : K;
}[keyof T & string];
